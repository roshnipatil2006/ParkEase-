...
def show_payment_window(plate, owner, vtype, checkin_time, checkout_time, amount):
    ...
    Label(frame, text="Payment Details", font=f1, bg="white", fg="#333").pack(pady=10)
    ...
    Label(frame, text=f"Check-in: {checkin_time}", font=f3, bg="white").pack(pady=5)
    Label(frame, text=f"Check-out: {checkout_time}", font=f3, bg="white").pack(pady=5)
    Label(frame, text=f"Amount to Pay: Rs.{amount}", font=f2, fg="green", bg="white").pack(pady=10)

    Button(frame, text="Pay Now", font=f4, bg="#4CAF50", fg="white", width=12, command=pay_win.destroy).pack(pady=15, ipady=5)
...
entry_plate = Entry(frame, font=f2, bd=2)
entry_plate.pack(pady=(0, 12), ipadx=5, ipady=5)
entry_plate.bind("<KeyRelease>", fetch_vehicle_details)

label_owner = Label(frame, text="Owner: ", font=f2, bg="#ffffff")
label_owner.pack(pady=(5, 5))
label_type = Label(frame, text="Vehicle Type: ", font=f2, bg="#ffffff")
label_type.pack(pady=(5, 15))

Button(frame, text="Check-out", font=f4, width=18, bg="#4CAF50", fg="white", command=checkout_vehicle).pack(pady=10, ipady=5)
